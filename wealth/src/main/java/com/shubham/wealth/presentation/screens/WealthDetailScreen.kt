...
fun WealthDetailScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.loadWealthDetails(coinId)
    }

    AppLogger.d(message = coin.toString())

    Scaffold(
        topBar = {
            AppToolbar(
                title = if (isLoading) "Coin Details" else coin?.name,
            ...
            if (isLoading) {
                ...
                coin?.let {
                    Column(
                        ...
                        TextComponent(
                            textValue = "Founder - ${coin!!.team[0].name}",
